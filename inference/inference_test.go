...
func testPackedCompressedEngineModel() (*model.Transformer, *tokenizer.Tokenizer) {
	config := model.Config{
		SequenceLen: 16, VocabSize: 32, NumLayer: 2, NumHead: 2, NumKVHead: 2,
		EmbedDim: 32, WindowPattern: "L", CompressionRatio: 2, QAT: "int8",
	}
	transformer := model.NewTransformer(config)
	transformer.InitWeights(tensors.NewRNG(42))
...
func TestEngineMatchesRecomputePackedCompressed(test *testing.T) {
	transformer, tokenizerImpl := testPackedCompressedEngineModel()
