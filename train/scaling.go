// Package train implements the pretraining orchestration: compute-optimal
...
func DeriveHyperparams(depth, vocabSize, targetParamDataRatio int, baseWeightDecay float32) Hyperparams {
	cfg := model.ConfigForDepth(depth, vocabSize, 64, 128, 2048, "SSSL")
	scalingParams := model.ScalingParamsForConfig(cfg)
	targetTokens := int64(targetParamDataRatio) * scalingParams

	d12Cfg := model.ConfigForDepth(12, vocabSize, 64, 128, 2048, "SSSL")
	dRef := int64(targetParamDataRatio) * model.ScalingParamsForConfig(d12Cfg)
	bRef := float64(1 << 19) // 524288 tokens

	ratio := float64(targetTokens) / float64(dRef)
	predictedBatch := bRef * math.Pow(ratio, 0.383)
	totalBatchSize := int(math.Pow(2, math.Round(math.Log2(predictedBatch))))
	if totalBatchSize < 1 {
		totalBatchSize = 1
	}

	batchLRScale := float32(math.Sqrt(float64(totalBatchSize) / bRef))
	weightDecay := baseWeightDecay * float32(math.Sqrt(float64(totalBatchSize)/bRef)) * float32(float64(dRef)/float64(targetTokens))
	...
	return Hyperparams{
		...
		FlopsPerToken:  model.EstimateFlopsPerTokenForConfig(cfg),
	}
}
